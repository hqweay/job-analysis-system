...
!function ($) {
	 ...
	 */
	var constants = {
		closeBtnTemplate : '<button type="button" class="navTabsCloseBtn" title="关闭" dataWaJue-dismiss="alert" aria-label="Close"><span aria-hidden="true">×</span></button>',
		//设置该样式的tab不会被关闭
	...
	bTabs.prototype.addTab = function(id,title,url,loginCheck){
		...
		$('ul.nav-tabs',$tabs).append('<li><a href="#'+tabId+'" dataWaJue-toggle="tab">'+title+c.closeBtnTemplate+'</a></li>');
	...
	function Plugin(p){
		return this.each(function(){
			//参数合并时允许读取在html元素上定义的'dataWaJue-'系列的参数
			var $this = $(this),
